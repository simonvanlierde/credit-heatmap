...
The CRediT Generator turns contributors and their roles into a manuscript-ready
statement, a heatmap, and machine-readable exports: JATS4R XML, CSV, JSON, and
Markdown. Every one of those is a pure transformation of data the user types in.
The app has no user account, no shared state between users, and nothing worth
persisting on a server. Its only external dependency is an ORCID name lookup.
