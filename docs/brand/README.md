...
The mark is a 3x3 crop of the contribution matrix, lead on the diagonal. Cell fills are the app's
...
| Asset | Source | Where it ships |
| --- | --- | --- |
| Favicon | `src/app/icon.svg` | Hand-written; carries its own dark-mode fills |
| Header lockup | `src/components/BrandMark.tsx` | Draws in `currentColor`, so it follows the theme |
| Apple touch icon | `apple-icon.html` | `src/app/apple-icon.png` |
| Social card | `opengraph-image.html` | `src/app/opengraph-image.png` |
| Maskable icon | `maskable-icon.html` | `public/icon-512.png`, via the web app manifest |
